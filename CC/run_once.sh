...
#ssh root@kahvefali "xl destroy $L1A_DOMNAME; xl destroy $L1B_DOMNAME; mount -o noacl,nocto,noatime,nodiratime  192.168.66.5:/xen /nfs; xl create /var/lib/nova/instances/nestedvirt/new_image/new_debian2.cfg; xl create /var/lib/nova/instances/nestedvirt/new_image/new_debian.cfg;"
#ssh root@kahvefali "mount -o noacl,nocto,noatime,nodiratime  192.168.66.5:/xen /nfs; xl create /nfs/configs/acentosbig.cfg"
...
bash launch_crash_detector.sh $L2_IP $L0_FLOW $L1A_ID $L1B_ID $L1B_IP &
CRSH_DET_PID=$!
...
timeout 120 ssh -o ConnectTimeout=60 root@$L2_IP "pwd; dmesg; date +%s;" &> responsive1.txt
