...
do
	...
	output=$(. ./run_once_performance.sh $CSV_MASTER_FILE $MIGRATION_SLEEP_TIME $INJ_REG $INJ_BIT $INJ_SLEEP)
	echo -e $output

	echo "Extracting dmesg"
	DMESG_LOG=`ssh root@kahvefali "xl dmesg | tail -n150"`
	...
	echo "$START,$REBOOT_DUR,$REBOOT_END,$PARSED,$INJ_REG,$INJ_BIT,$INJ_SLEEP,$FI_CSV,$MIGR_CSV,$PING_PATH_L2,$PING_PATH_L1A,$PING_PATH_L1B,$PING_PATH_L0,$SAR_LOG_PATH,$CSV_PATH" >> $CSV_MASTER_FILE
done
losetup -d /dev/loop0
