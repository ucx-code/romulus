...
void handle_xc_stream(int fp, int full_scan)
{
...
int main(int argc, char * argv[])
{
    ...
    assert(fp >= 0);
    if (argc == 3) {
	sscanf(argv[2], "%d", &full_scan);
    } else {
	full_scan = 0; // Stop when we reach top of xc stream
    }
    ...
    while (1) {
        ...
        switch (libxl_rec.type)
        {
            ...
            case REC_TYPE_LIBXC_CONTEXT:
                handle_xc_stream(fp, full_scan);
		if (!full_scan)
			goto force_stream_end;
                break;
