...
void swap_nvmcs(struct hvm_hw_cpu * source, struct hvm_hw_cpu * target)
{
	SWAP(source, target, cs_sel);
	SWAP(source, target, ds_sel);
	SWAP(source, target, es_sel);
	SWAP(source, target, fs_sel);
	SWAP(source, target, gs_sel);
	SWAP(source, target, ss_sel);
	SWAP(source, target, tr_sel);
	SWAP(source, target, ldtr_sel);
	SWAP(source, target, cs_limit);
	SWAP(source, target, ds_limit);
	SWAP(source, target, es_limit);
	SWAP(source, target, fs_limit);
	SWAP(source, target, gs_limit);
	SWAP(source, target, ss_limit);
	SWAP(source, target, tr_limit);
	SWAP(source, target, ldtr_limit);
	SWAP(source, target, idtr_limit);
	SWAP(source, target, gdtr_limit);
	SWAP(source, target, cs_base);
	SWAP(source, target, ds_base);
	SWAP(source, target, es_base);
	SWAP(source, target, fs_base);
	SWAP(source, target, gs_base);
	SWAP(source, target, ss_base);
	SWAP(source, target, tr_base);
	SWAP(source, target, ldtr_base);
	SWAP(source, target, idtr_base);
	SWAP(source, target, gdtr_base);
	SWAP(source, target, cs_arbytes);
	SWAP(source, target, ds_arbytes);
	SWAP(source, target, es_arbytes);
	SWAP(source, target, fs_arbytes);
	SWAP(source, target, gs_arbytes);
	SWAP(source, target, ss_arbytes);
	SWAP(source, target, tr_arbytes);
	SWAP(source, target, ldtr_arbytes);
	SWAP(source, target, sysenter_cs);
	SWAP(source, target, sysenter_esp);
	SWAP(source, target, sysenter_eip);
}
...
struct hvm_hw_cpu * extract_nvmcs(xmlNodePtr cur, void * base)
{
    ...
    do {
    	val = xmlNodeGetContent(cur);
        if (xmlStrcmp(cur->name, (const xmlChar *) "es_sel") == 0) {
        	sscanf((const char *) val, "%u", &tmp->es_sel);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "cs_sel") == 0) {
        	sscanf((const char *) val, "%u", &tmp->cs_sel);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ss_sel") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ss_sel);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ds_sel") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ds_sel);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "fs_sel") == 0) {
        	sscanf((const char *) val, "%u", &tmp->fs_sel);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "gs_sel") == 0) {
        	sscanf((const char *) val, "%u", &tmp->gs_sel);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "tr_sel") == 0) {
        	sscanf((const char *) val, "%u", &tmp->tr_sel);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ldtr_sel") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ldtr_sel);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "es_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->es_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "cs_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->cs_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ss_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ss_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ds_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ds_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "fs_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->fs_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "gs_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->gs_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "tr_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->tr_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ldtr_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ldtr_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "idtr_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->idtr_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "gdtr_limit") == 0) {
        	sscanf((const char *) val, "%u", &tmp->gdtr_limit);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "es_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->es_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "cs_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->cs_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ss_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->ss_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ds_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->ds_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "fs_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->fs_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "gs_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->gs_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "tr_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->tr_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ldtr_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->ldtr_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "idtr_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->idtr_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "gdtr_base") == 0) {
        	sscanf((const char *) val, "%lu", &tmp->gdtr_base);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "es_arbytes") == 0) {
        	sscanf((const char *) val, "%u", &tmp->es_arbytes);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "cs_arbytes") == 0) {
        	sscanf((const char *) val, "%u", &tmp->cs_arbytes);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ss_arbytes") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ss_arbytes);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ds_arbytes") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ds_arbytes);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "fs_arbytes") == 0) {
        	sscanf((const char *) val, "%u", &tmp->fs_arbytes);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "gs_arbytes") == 0) {
        	sscanf((const char *) val, "%u", &tmp->gs_arbytes);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "tr_arbytes") == 0) {
        	sscanf((const char *) val, "%u", &tmp->tr_arbytes);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "ldtr_arbytes") == 0) {
        	sscanf((const char *) val, "%u", &tmp->ldtr_arbytes);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "sysenter_cs") == 0) {
                sscanf((const char *) val, "%lu", &tmp->sysenter_cs);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "sysenter_esp") == 0) {
                sscanf((const char *) val, "%lu", &tmp->sysenter_esp);
	} else if (xmlStrcmp(cur->name, (const xmlChar *) "sysenter_eip") == 0) {
                sscanf((const char *) val, "%lu", &tmp->sysenter_eip);
	} else {
		//assert(0); // BUG
	}
	...
	assert((xmlDoc = read_xml_file(base_xml_file)) != NULL);
 	...
 	switch (structure) {
		...
		case 5: /* nVMCS (CPU) */
			lseek(base_fp, offset, SEEK_SET);
 			assert(read(base_fp, &tmp_cpu, sizeof(struct hvm_hw_cpu)) > 0);
			new_cpu = (struct hvm_hw_cpu *) extract_from_xml((void *) extract_nvmcs, xmlDoc, BAD_CAST "/nvmcs", (void *) &tmp_cpu);
			swap_nvmcs(&tmp_cpu, new_cpu);
		        lseek(base_fp, offset, SEEK_SET);
			write(base_fp, &tmp_cpu, sizeof(tmp_cpu));
			free(new_cpu);
		...
		case 4: /* Remove all page data entries from xc stream */
		    ...
		    new_fp = open(new_file_path, O_RDWR | O_CREAT, 0666);
